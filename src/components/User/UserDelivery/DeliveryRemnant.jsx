...
//                   </div>
//                   <div className="delrem-stat-card-content">
//                     <div className="delrem-stat-card-value">{formatNumber(remnant.accumulatedKg)}kg</div>
//                     <div className="delrem-stat-card-subtitle">
//                       {(remnant.accumulatedKg || 0) >= 6 ? (
//                         <span className="delrem-status-success">
//                           <FaCheckCircle /> Eligible for delivery
//                         </span>
//                       ) : (
//                         <span className="delrem-status-warning">
//                           <FaExclamationTriangle /> Needs {formatNumber(6 - (remnant.accumulatedKg || 0))}kg more
//                         </span>
//                       )}
//                     </div>
//                   </div>
...
//                   </div>
//                   <div className="delrem-stat-card-content">
//                     <div className="delrem-stat-card-value">{formatNumber(remnant.deliveredFromRemnant)}kg</div>
//                     <div className="delrem-stat-card-subtitle">
//                       Total delivered from remnant
//                     </div>
//                   </div>
...
//                             <div className="delrem-entry-date">{formatDate(entry.date)}</div>
//                             <div className="delrem-entry-amount">
//                               {formatNumber(entry.delivered)}kg delivered, {formatNumber(entry.remaining)}kg added to remnant
//                             </div>
//                           </div>
//                           <button
//                             className="delrem-confirm-entry-btn"
...
//                       <div className="delrem-quick-action-subtitle">
//                         {(remnant.accumulatedKg || 0) >= 6
//                           ? `Request delivery of your ${formatNumber(remnant.accumulatedKg)}kg remnant`
//                           : `Minimum ${formatNumber(6 - (remnant.accumulatedKg || 0))}kg needed`}
//                       </div>
//                     </div>
//                     <FaChevronRight className="delrem-quick-action-arrow" />
//                   </button>
...
//                         <div
//                           className="delrem-progress-fill"
//                           style={{ width: `${Math.min(((remnant.accumulatedKg || 0) / 50) * 100, 100)}%` }}
//                         >
//                           <span className="delrem-progress-label">
//                             {formatNumber(remnant.accumulatedKg)}kg accumulated
//                           </span>
//                         </div>
//                       </div>
//                       <div className="delrem-progress-metrics">
...
//                         <div className="delrem-progress-metric">
//                           <span className="delrem-metric-label">Current Available</span>
//                           <span className="delrem-metric-value">{formatNumber(remnant.accumulatedKg)}kg</span>
//                         </div>
//                         <div className="delrem-progress-metric">
//                           <span className="delrem-metric-label">Next Delivery At</span>
//                           <span className="delrem-metric-value">{formatNumber(Math.max(6 - (remnant.accumulatedKg || 0), 0))}kg more</span>
//                         </div>
//                       </div>
//                     </div>
...
//                             <div className="delrem-activity-content">
//                               <div className="delrem-activity-title">
//                                 Partial Delivery - {formatNumber(entry.delivered)}kg delivered
//                               </div>
//                               <div className="delrem-activity-subtitle">
//                                 {formatDate(entry.date)} • Added {formatNumber(entry.remaining)}kg to remnant
//                               </div>
//                             </div>
//                             <div className={`delrem-activity-status ${entry.confirmed ? 'delrem-confirmed' : 'delrem-pending'}`}>
//                               {entry.confirmed ? 'Confirmed' : 'Pending'}
...
//                             <tr key={index} className={entry.confirmed ? 'delrem-confirmed-row' : 'delrem-pending-row'}>
//                               <td>{formatDate(entry.date)}</td>
//                               <td>{formatNumber(entry.originalKg)}kg</td>
//                               <td>{formatNumber(entry.delivered)}kg</td>
//                               <td className="delrem-success">{formatNumber(entry.remaining)}kg</td>
//                               <td>
//                                 <span className={`delrem-status-badge ${entry.confirmed ? 'delrem-confirmed' : 'delrem-pending'}`}>
//                                   {entry.confirmed ? 'Confirmed' : 'Pending'}
//                                 </span>
...
//             <div className="delrem-modal-content">
//               <div className="delrem-modal-info">
//                 <p><strong>Available Remnant:</strong> {formatNumber(remnant?.accumulatedKg)}kg</p>
//                 <p><strong>Minimum Delivery:</strong> 6kg</p>
//                 {pendingEntries.length > 0 && (
//                   <div className="delrem-modal-warning">
//                     <FaExclamationTriangle />
...
//                       max={remnant?.accumulatedKg}
//                       step="0.5"
//                       placeholder={`Enter amount (6-${formatNumber(remnant?.accumulatedKg)}kg)`}
//                       disabled={pendingEntries.length > 0}
//                     />
//                     <span className="delrem-input-suffix">kg</span>
//                   </div>
...
//                     <div className="delrem-entry-date">{formatDate(entry.date)}</div>
//                     <div className="delrem-entry-details">
//                       <span>{formatNumber(entry.delivered)}kg delivered, </span>
//                       <span className="delrem-entry-added">+{formatNumber(entry.remaining)}kg added to remnant</span>
//                     </div>
//                     <div className="delrem-entry-original">Original order: {formatNumber(entry.originalKg)}kg</div>
//                   </div>
//                 ))}
//               </div>
...
const CustomerRemnant = () => {
  ...
  const fetchRemnant = async () => {
    try {
      setLoading(true);
      const response = await ApiService.delivery.getMyRemnant();
      const data = response.data;
      if (data.success) {
        if (data.data && data.data.current) setRemnant(data.data.current);
        else if (data.data) setRemnant(data.data);
        else setRemnant(null);
      } else {
        showSnackbar(
          data.message || "Error fetching remnant data",
          "error",
        );
      }
    } catch (error) {
  ...
  const handleConfirmEntry = async () => {
    ...
    try {
      setLoading(true);
      const response = await ApiService.delivery.confirmRemnant(
        remnant._id,
        confirmationNotes,
      );
      const data = response.data;
  ...
  const handleRequestDelivery = async () => {
    ...
    if (requested > accumulated) {
      showSnackbar(
        `Cannot request more than ${accumulated}kg available`,
        "error",
      );
      return;
    }
    ...
    const unconfirmedEntries =
      remnant?.partialDeliveries?.filter((p) => !p.confirmed) || [];
    if (unconfirmedEntries.length > 0) {
      showSnackbar(
        `Please confirm ${unconfirmedEntries.length} pending remnant entries first`,
        "error",
      );
      return;
    }
    ...
    try {
      setLoading(true);
      const response = await ApiService.delivery.requestRemnantDelivery({
        requestedKg: requested,
        deliveryDate: deliveryDate || new Date().toISOString().split("T")[0],
        notes: deliveryNotes,
      });
  ...
  const canRequestDelivery = () => {
    ...
    const hasEnoughKg = (remnant.accumulatedKg || 0) >= 6;
    const isConfirmed = remnant.customerConfirmation?.confirmed === true;
    const noPendingEntries =
      (remnant.partialDeliveries?.filter((p) => !p.confirmed) || []).length ===
      0;
    return hasEnoughKg && isConfirmed && noPendingEntries;
  };
  ...
  const getRequestDisabledReason = () => {
    if (!remnant) return "No remnant data";
    if ((remnant.accumulatedKg || 0) < 6)
      return `Need ${formatNumber(6 - (remnant.accumulatedKg || 0))}kg more`;
    if (!remnant.customerConfirmation?.confirmed)
      return "Confirm remnant entries first";
    const pendingCount = (
      remnant.partialDeliveries?.filter((p) => !p.confirmed) || []
    ).length;
    if (pendingCount > 0) return `${pendingCount} entries pending confirmation`;
    ...
    <div className="delrem-remnant-container">
      <div className="delrem-remnant-main-content">
        <div className="delrem-content-header">
          <div className="delrem-header-left">
            <h2 className="delrem-content-title">
              <FaGasPump className="delrem-title-icon" /> My Gas Remnant
            </h2>
            <div className="delrem-content-subtitle">
              Manage your accumulated gas remnant and delivery requests
            </div>
          ...
          <div className="delrem-header-right">
            <button
              ...
              title={getRequestDisabledReason()}
            >
              <FaShoppingCart /> Request Delivery
            </button>
          </div>
        </div>
        <div className="delrem-secondary-tabs">
            ...
            <>
              <div className="delrem-top-stats-cards">
                <div className="delrem-stat-card delrem-stat-card-primary">
                  <div className="delrem-stat-card-header">
                    <FaGasPump className="delrem-stat-card-icon" />
                    <span className="delrem-stat-card-title">
                      Accumulated Gas
                    </span>
                  </div>
                  <div className="delrem-stat-card-content">
                    <div className="delrem-stat-card-value">
                      {formatNumber(remnant.accumulatedKg)}kg
                    </div>
                    <div className="delrem-stat-card-subtitle">
                      {(remnant.accumulatedKg || 0) >= 6 ? (
                        <span className="delrem-status-success">
                        ...
                        <span className="delrem-status-warning">
                          <FaExclamationTriangle /> Needs{" "}
                          {formatNumber(6 - (remnant.accumulatedKg || 0))}kg
                          more
                        </span>
                      )}
                    </div>
                ...
                <div className="delrem-stat-card">
                  ...
                  <div className="delrem-stat-card-content">
                    <div className="delrem-stat-card-value">
                      {formatNumber(remnant.deliveredFromRemnant)}kg
                    </div>
                    <div className="delrem-stat-card-subtitle">
                      Total delivered from remnant
                    </div>
                ...
                <div className="delrem-stat-card">
                  ...
                  <div className="delrem-stat-card-content">
                    <div className="delrem-stat-card-value">
                      {remnant.deliveryRequests?.filter(
                        (r) => r.status === "pending",
                      ).length || 0}
                    </div>
                    <div className="delrem-stat-card-subtitle">
              ...
              {pendingEntries.length > 0 && (
                <div className="delrem-confirmation-required">
                  ...
                  <div className="delrem-confirmation-content">
                    <p>
                      You have {pendingEntries.length} unconfirmed partial
                      delivery entries. Please confirm them before requesting
                      remnant delivery.
                    </p>
                    <div className="delrem-pending-entries">
                      {pendingEntries.slice(0, 3).map((entry, index) => (
                        <div key={index} className="delrem-pending-entry">
                          <FaGasPump className="delrem-entry-icon" />
                          <div className="delrem-entry-details">
                            <div className="delrem-entry-date">
                              {formatDate(entry.date)}
                            </div>
                            <div className="delrem-entry-amount">
                              {formatNumber(entry.delivered)}kg delivered,{" "}
                              {formatNumber(entry.remaining)}kg added to remnant
                            </div>
                          </div>
                          <button
                            className="delrem-confirm-entry-btn"
                            onClick={() =>
                              handleConfirmPartialDelivery(
                                entry.deliveryId?._id || entry.deliveryId,
                              )
                            }
                            disabled={!entry.deliveryId}
                          >
              ...
              <div className="delrem-quick-actions-section">
                ...
                <div className="delrem-quick-actions-grid">
                  ...
                  >
                    ...
                    <div className="delrem-quick-action-content">
                      <div className="delrem-quick-action-title">
                        Request Delivery
                      </div>
                      <div className="delrem-quick-action-subtitle">
                        {(remnant.accumulatedKg || 0) >= 6
                  ...
                  <button
                    ...
                    disabled={pendingEntries.length === 0}
                    title={
                      pendingEntries.length === 0 ? "No entries to confirm" : ""
                    }
                  >
                    <div className="delrem-quick-action-icon delrem-success">
                      <FaCheckCircle />
                ...
                {activeTab === "overview" && (
                  <div className="delrem-overview-content">
                    <div className="delrem-progress-section">
                      <div className="delrem-progress-header">
                        <h4>Remnant Progress</h4>
                        <span className="delrem-progress-target">
                          Target: 50kg
                        </span>
                      </div>
                      <div className="delrem-progress-bar-large">
                        <div
                          className="delrem-progress-fill"
                          style={{
                            width: `${Math.min(((remnant.accumulatedKg || 0) / 50) * 100, 100)}%`,
                          }}
                        >
                          <span className="delrem-progress-label">
                            {formatNumber(remnant.accumulatedKg)}kg accumulated
                      ...
                      <div className="delrem-progress-metrics">
                        <div className="delrem-progress-metric">
                          <span className="delrem-metric-label">
                            Minimum Delivery
                          </span>
                          <span className="delrem-metric-value">6kg</span>
                        </div>
                        <div className="delrem-progress-metric">
                          ...
                          <span className="delrem-metric-value">
                            {formatNumber(remnant.accumulatedKg)}kg
                          </span>
                        </div>
                        <div className="delrem-progress-metric">
                          ...
                          <span className="delrem-metric-value">
                            {formatNumber(
                              Math.max(6 - (remnant.accumulatedKg || 0), 0),
                            )}
                            kg more
                          </span>
                        </div>
                    ...
                    <div className="delrem-recent-activity">
                      ...
                      <div className="delrem-activity-list">
                        {remnant.partialDeliveries
                          ?.slice(0, 5)
                          .map((entry, index) => (
                            <div key={index} className="delrem-activity-item">
                              <div
                                className={`delrem-activity-icon ${entry.confirmed ? "delrem-confirmed" : "delrem-pending"}`}
                              >
                                {entry.confirmed ? (
                                  <FaCheckCircle />
                                ) : (
                              ...
                              <div className="delrem-activity-content">
                                ...
                                <div className="delrem-activity-subtitle">
                                  {formatDate(entry.date)} • Added{" "}
                                  {formatNumber(entry.remaining)}kg to remnant
                                </div>
                              </div>
                              <div
                                className={`delrem-activity-status ${entry.confirmed ? "delrem-confirmed" : "delrem-pending"}`}
                              >
                                {entry.confirmed ? "Confirmed" : "Pending"}
                              </div>
                            </div>
                        ...
                        <tbody>
                          {remnant.partialDeliveries?.map((entry, index) => (
                            ...
                            >
                              ...
                              <td>{formatNumber(entry.originalKg)}kg</td>
                              <td>{formatNumber(entry.delivered)}kg</td>
                              <td className="delrem-success">
                                {formatNumber(entry.remaining)}kg
                              </td>
                              <td>
                                <span
                                  className={`delrem-status-badge ${entry.confirmed ? "delrem-confirmed" : "delrem-pending"}`}
                                >
                                  {entry.confirmed ? "Confirmed" : "Pending"}
                                </span>
                              </td>
                        ...
                        <tbody>
                          {remnant.deliveryRequests?.map((request, index) => (
                            <tr key={index}>
                              ...
                              <td>
                                <span
                                  className={`delrem-status-badge ${request.status}`}
                                >
                                  {request.status?.toUpperCase() || "PENDING"}
                                </span>
                              </td>
                              <td>
                                {request.deliveryId?.status ? (
                                  <span
                                    className={`delrem-status-badge ${request.deliveryId.status}`}
                                  >
                                    {request.deliveryId.status.toUpperCase()}
                                  </span>
                                ) : (
                              ...
                              <td>
                                <div className="delrem-table-actions">
                                  <button
                                    className="delrem-action-btn delrem-view-btn"
                                    onClick={() =>
                                      (window.location.href = `/dashboard/delivery/${request.deliveryId?._id}`)
                                    }
                                  >
                                    <FaEye /> View
                                  </button>
      ...
      {requestDialogOpen && (
        <div className="delrem-modal-overlay">
          <div className="delrem-modal">
            <div className="delrem-modal-header">
              <h3>
                <FaShoppingCart /> Request Remnant Delivery
              </h3>
              <button
                className="delrem-modal-close"
                onClick={() => setRequestDialogOpen(false)}
            ...
            <div className="delrem-modal-content">
              <div className="delrem-modal-info">
                <p>
                  <strong>Available Remnant:</strong>{" "}
                  {formatNumber(remnant?.accumulatedKg)}kg
                </p>
                <p>
                ...
                {pendingEntries.length > 0 && (
                  <div className="delrem-modal-warning">
                    <FaExclamationTriangle />
                    <p>
                      You have {pendingEntries.length} unconfirmed entries.
                      Please confirm them first.
                    </p>
                  </div>
                )}
              </div>
      ...
      {confirmDialogOpen && (
        <div className="delrem-modal-overlay">
          <div className="delrem-modal">
            <div className="delrem-modal-header">
              <h3>
                <FaCheckCircle /> Confirm Remnant Entries
              </h3>
              <button
                className="delrem-modal-close"
                onClick={() => setConfirmDialogOpen(false)}
            ...
            <div className="delrem-modal-content">
              <div className="delrem-modal-alert">
                <FaExclamationTriangle />
                <p>
                  Please confirm all your accumulated remnant entries. This is
                  required before requesting remnant delivery.
                </p>
              </div>
              <div className="delrem-entries-list">
                {pendingEntries.map((entry, index) => (
                  <div key={index} className="delrem-entry-item">
                    ...
                    <div className="delrem-entry-details">
                      <span>{formatNumber(entry.delivered)}kg delivered, </span>
                      <span className="delrem-entry-added">
                        +{formatNumber(entry.remaining)}kg added to remnant
                      </span>
                    </div>
                    <div className="delrem-entry-original">
                      Original order: {formatNumber(entry.originalKg)}kg
                    </div>
                  </div>
                ))}
              </div>
