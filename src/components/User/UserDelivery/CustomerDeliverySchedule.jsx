...
//       (sub) => sub._id === delivery.subscriptionId?._id
//     );
//   };
...
//       if (data.success) {
//         showSnackbar(
//           "Subscription and deliveries paused successfully",
//           "success"
//         );
//         fetchSubscriptions();
//         fetchDeliveries();
...
//       if (data.success) {
//         showSnackbar(
//           "Subscription and deliveries resumed successfully",
//           "success"
//         );
//         fetchSubscriptions();
//         fetchDeliveries();
...
//                   <span>
//                     Delivery is out of sync with subscription (
//                     {subscription.status})
//                   </span>
//                   <button
...
//               {isPartialDelivery && (
//                 <div className="adm-partial-delivery-badge">
...
//                         <span className="adm-delivered-amount">
//                           <FaCheckCircle className="adm-icon-sm" /> Delivered: {delivery.deliveredKg}kg
//                         </span>
//                         <span className="adm-remaining-amount">
//                           <FaGasPump className="adm-icon-sm" /> Added to remnant: {delivery.remainingKg}kg
//                         </span>
//                       </div>
...
//                           ` (since ${formatDate(subscription.pausedAt)})`}
//                       </span>
//                     </div>
//                   )}
...
//                         {" "}
//                         Delivery paused{" "}
//                         {subscription ? "due to subscription pause" : ""}
//                       </strong>
//                       {delivery.pausedAt && (
...
//                           {formatDateShort(
//                             new Date(
//                               delivery.originalDeliveryDate.getTime() +
//                                 (subscription?.pauseHistory?.reduce(
//                                   (total, pause) =>
//                                     total + (pause.durationMs || 0),
//                                   0
//                                 ) || 0)
//                             )
//                           )}
...
//   const [statusFilter, setStatusFilter] = useState("all");
//   const [dateFilter, setDateFilter] = useState("");
//   const [sortOrder, setSortOrder] = useState("desc"); // Changed default to desc (newest first)
...
//   // Fetch deliveries when any filter changes
//   useEffect(() => {
//     fetchDeliveries();
//   }, [page, activeTab, statusFilter, dateFilter, sortOrder, sortBy]);
...
//       // Add tab parameter (this ensures each tab gets ONLY its data)
//       if (activeTab) {
//         queryParams.append("tab", activeTab);
//       }

//       // Add status filter only if not using a tab that already filters status
...
//     setStatusFilter("all");
//     setDateFilter("");
//     // Don't reset sort settings
//   };

//   const handleStatusFilterChange = (status) => {
//     setStatusFilter(status);
//     setActiveTab(null); // Clear tab when using custom filter
//     setPage(1);
//   };
...
//         {/* Tabs - each tab now gets its own data from backend */}
//         <div className="adm-tabs">
//           <button
//             className={`adm-tab ${activeTab === "upcoming" ? "active" : ""}`}
//             onClick={() => handleTabChange("upcoming")}
...
//               <span className="adm-tab-count">{tabCounts.upcoming}</span>
//             )}
//           </button>
//           <button
//             className={`adm-tab ${activeTab === "delivered" ? "active" : ""}`}
...
//               <span className="adm-tab-count">{tabCounts.delivered}</span>
//             )}
//           </button>
//           <button
//             className={`adm-tab ${activeTab === "overdue" ? "active" : ""}`}
...
//             Overdue
//             {tabCounts.overdue > 0 && (
//               <span className="adm-tab-count adm-tab-count-warning">{tabCounts.overdue}</span>
//             )}
//           </button>
...
//           {/* Status Filter - Only show when not using tabs */}
//           {!activeTab && (
//             <div className="adm-filter-group">
//               <FaFilter className="adm-control-icon" />
//               <select
...
//           {/* Clear Filters */}
//           {(statusFilter !== "all" || dateFilter || activeTab !== "upcoming" || sortBy !== "deliveryDate" || sortOrder !== "desc") && (
//             <button className="adm-clear-filters" onClick={clearFilters}>
//               Clear Filters
//             </button>
//           )}
...
//         <div className="adm-active-tab-info">
//           Showing: <strong>{activeTab ? activeTab.charAt(0).toUpperCase() + activeTab.slice(1) : 'All'} Deliveries</strong>
//           {sortBy === 'deliveryDate' && (
//             <span> • Sorted by Date: {sortOrder === 'asc' ? 'Oldest First' : 'Newest First'}</span>
//           )}
//         </div>
//       </div>
...
//                 ? "You don't have any delivered orders."
//                 : activeTab === "overdue"
//                 ? "You don't have any overdue deliveries. Great job!"
//                 : "You don't have any delivery records for the selected filter."}
//             </p>
//             {(statusFilter !== "all" || dateFilter || activeTab !== "upcoming" || sortBy !== "deliveryDate" || sortOrder !== "desc") && (
//               <button
...
//         <div className="adm-pagination">
//           <div className="adm-pagination-info">
//             Page {page} of {totalPages} • Showing {deliveries.length} of {totalCount} deliveries
//           </div>
//           <div className="adm-pagination-buttons">
//             <button
//               className="adm-page-btn"
...
// src/pages/CustomerDeliverySchedule.jsx
import React, { useState, useEffect } from "react";
import { Link } from "react-router-dom";
...
const CustomerDeliveryHistory = () => {
  ...
  const fetchSubscriptions = async () => {
    try {
      const response = await ApiService.subscriptions.getMySubscriptions();
  ...
  const fetchDeliveries = async () => {
    try {
      ...
      if (!activeTab && statusFilter !== "all")
        queryParams.append("status", statusFilter);
      if (dateFilter) queryParams.append("deliveryDate", dateFilter);

      const response = await ApiService.delivery.getMyDeliveries(
  ...
  const getDeliverySubscription = (delivery) =>
    subscriptions.find((sub) => sub._id === delivery.subscriptionId?._id);
  ...
  const syncDeliveryWithSubscription = async (deliveryId) => {
    try {
      const response =
        await ApiService.delivery.syncWithSubscription(deliveryId);
      ...
      if (data.success) {
        ...
        showSnackbar(response.message || "Sync failed", "error");
      }
    } catch (error) {
      showSnackbar("Error syncing delivery", "error");
    }
  ...
  const handleConfirmDelivery = async () => {
    try {
      const response = await ApiService.delivery.confirmDelivery(
  ...
  const getDeliveryTimeContext = (delivery) => {
    ...
    const deliveryDate = new Date(delivery.deliveryDate);
    deliveryDate.setHours(0, 0, 0, 0);
    const daysDiff = Math.floor((deliveryDate - now) / (1000 * 60 * 60 * 24));
  ...
  return (
    <div className="adm-customer-delivery">
      <div className="adm-customer-header">
        ...
        <div className="adm-tabs">
          ...
          >
            Overdue
            {tabCounts.overdue > 0 && (
              <span className="adm-tab-count adm-tab-count-warning">
                {tabCounts.overdue}
              </span>
            )}
          </button>
                ...
                <div className="adm-card-header">
                  ...
                  <span className="adm-delivery-date">
                    ...
                    {delivery.originalDeliveryDate && (
                      <span className="adm-original-date">
                        (Originally:{" "}
                        {formatDateShort(delivery.originalDeliveryDate)})
                      </span>
                    )}
                  </span>
                </div>
                <div className="adm-plan-info">
                  ...
                  {isPartialDelivery && (
                    <div className="adm-partial-delivery-details">
                      <div className="adm-partial-breakdown">
                        <span className="adm-delivered-amount">
                          <FaCheckCircle className="adm-icon-sm" /> Delivered:{" "}
                          {delivery.deliveredKg}kg
                        </span>
                        <span className="adm-remaining-amount">
                          <FaGasPump className="adm-icon-sm" /> Added to
                          remnant: {delivery.remainingKg}kg
                        </span>
                      </div>
                      <div className="adm-remnant-note">
                        <FaInfoCircle className="adm-icon-sm" />
                        Please confirm this remnant entry in your Remnant
                        Dashboard
                      </div>
                    </div>
                  )}
                ...
                <div className="adm-card-footer">
                  ...
                  {delivery.status === "delivered" &&
                    ...
                    !isPartialDelivery && (
                      ...
                      >
                        <FaCheckCircle className="adm-icon" /> Confirm Delivery
                      </button>
                    )}
                  {syncStatus.shouldBeSynced && (
                    <button
                      ...
                      onClick={() => syncDeliveryWithSubscription(delivery._id)}
                    >
                      <FaSync className="adm-icon" /> Sync with Subscription
                    </button>
                  )}
                  {isPartialDelivery &&
                    !delivery.customerConfirmation?.confirmed && (
                      <Link
                        ...
                        className="adm-btn adm-btn-warning"
                      >
                        <FaGasPump className="adm-icon" /> Confirm Remnant Entry
                      </Link>
                    )}
                </div>
              </div>
      ...
      {totalPages > 1 && (
        <div className="adm-pagination">
          <div className="adm-pagination-info">
            Page {page} of {totalPages} • Showing {deliveries.length} of{" "}
          ...
          <div className="adm-dialog">
            ...
            <div className="adm-dialog-footer">
              ...
              <button
                ...
                onClick={handleConfirmDelivery}
              >
                <FaCheckCircle className="adm-icon" /> Confirm Receipt
              </button>
            </div>
          </div>
        </div>
