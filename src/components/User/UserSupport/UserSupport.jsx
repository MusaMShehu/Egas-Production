...
const Support = () => {
  ...
  const fetchTickets = async () => {
    ...
    try {
      ...
      if (ticketData.length === 0) {
        ...
        successToast(
          `Loaded ${ticketData.length} support tickets successfully`,
        );
      }
    } catch (err) {
      console.error("Error fetching tickets:", err);
  ...
  const handleAddResponse = async (id) => {
    ...
    try {
      ...
      const response = await ApiService.support.addResponse(id, formData);
      const updatedTicket = await ApiService.support.getById(id);
      setTickets((prev) =>
        prev.map((t) => (t._id === id ? updatedTicket.data : t)),
      );
      if (selectedTicket && selectedTicket._id === id) {
  ...
  const handleViewTicketDetails = async (ticket) => {
    infoToast(`Loading ticket #${ticket.ticketId}...`);
    try {
      const ticketDetails = await ApiService.support.getById(ticket._id);
      setSelectedTicket(ticketDetails.data);
    } catch (err) {
      console.error("Error loading ticket details:", err);
      setSelectedTicket(ticket);
      warningToast("Showing cached ticket data. Some details may be outdated.");
  ...
  const closeTicket = async (id) => {
    ...
    warningToast(`Closing ticket #${ticket.ticketId}...`);
    try {
      const response = await ApiService.support.close(id);
  ...
  const getAttachmentName = (attachment) => {
    if (typeof attachment === "string")
      return attachment.split("/").pop() || "Attachment";
    return attachment?.original_filename || "Attachment";
  };
  ...
  const getFileIcon = (filename) => {
    ...
    if (["jpg", "jpeg", "png", "gif", "webp", "bmp"].includes(ext))
      return <FaImage />;
    if (["pdf"].includes(ext)) return <FaFile className="pdf-icon" />;
    return <FaFile />;
  };
              ...
              {filteredTickets.map((ticket) => (
                <div key={ticket._id} className="sup-ticket-card">
                  <div className="sup-ticket-header">
                    ...
                    <div
                      className={`sup-ticket-status ${getStatusClass(ticket.status)}`}
                    >
                      {getStatusText(ticket.status)}
                    </div>
                  </div>
                  ...
                  <div className="sup-ticket-meta">
                    ...
                    {ticket.images && ticket.images.length > 0 && (
                      <div className="sup-ticket-attachments">
                        <FaImage className="sup-fas" /> {ticket.images.length}{" "}
                        image(s)
                      </div>
                    )}
                  </div>
                  ...
                  <div className="sup-ticket-conversation">
                    ...
                    {selectedTicket.responses &&
                      selectedTicket.responses.map((response, index) => {
                        const isUser =
                          response.user && response.user._id === user?.id;
                        return (
                          <div
                            key={response._id || index}
                            className={`sup-message ${isUser ? "user-message" : "agent-message"}`}
                          >
                            <div className="sup-message-header">
                              <span className="sup-message-sender">
                                {isUser
                              ...
                              {responseAttachments.map((file, index) => (
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    className="sup-remove-attachment"
                                    onClick={() =>
                                      handleRemoveResponseAttachment(index)
                                    }
                                    disabled={isSubmitting}
                                  >
                                    <FaTrash />
              ...
              <div className="sup-support-modal-body">
                ...
                <div className="sup-form-group">
                  ...
                  {newTicket.images.length > 0 && (
                    <div className="sup-attachments-preview">
                      <strong>
                        Selected files ({newTicket.images.length}):
                      </strong>
                      {newTicket.images.map((f, i) => (
                        <div key={i} className="sup-attachment-item">
                          {getFileIcon(f.name)}
