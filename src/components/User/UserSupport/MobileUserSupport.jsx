...
const MobileSupport = () => {
  ...
  const handleAddResponse = async (ticketId) => {
    ...
    try {
      ...
      setTickets((prev) =>
        prev.map((t) => (t._id === ticketId ? updatedTicket.data : t)),
      );
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case "open":
        return (
          <FaExclamationCircle className="mobsup-status-icon mobsup-open" />
        );
      case "in-progress":
        return <FaClock className="mobsup-status-icon mobsup-in-progress" />;
      case "resolved":
        return <FaCheckCircle className="mobsup-status-icon mobsup-resolved" />;
      case "closed":
        return <FaCheckCircle className="mobsup-status-icon mobsup-closed" />;
      default:
        return (
          <FaExclamationCircle className="mobsup-status-icon mobsup-open" />
        );
            ...
            {filteredTickets.map((ticket) => (
              ...
              >
                <div className="mobsup-ticket-header">
                  ...
                  <div
                    className={`mobsup-ticket-status mobsup-${ticket.status}`}
                  >
              ...
              <div className="mobsup-ticket-info-section">
                <div className="mobsup-info-grid">
                  ...
                  <div className="mobsup-info-item">
                    ...
                    <span
                      className={`mobsup-value mobsup-status mobsup-${selectedTicket.status}`}
                    >
                      ...
                      {responseAttachments.length > 0 && (
                        <div className="mobsup-attachments-preview">
                          {responseAttachments.map((file, index) => (
                            <div
                              key={index}
                              className="mobsup-attachment-preview"
                            >
