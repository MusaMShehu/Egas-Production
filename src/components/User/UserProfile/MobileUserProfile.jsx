...
// src/pages/MobileUserProfile.jsx
import React, { useState, useEffect } from "react";
...
const ProfileMobile = () => {
  ...
  const handlePreferenceChange = (e) => {
    ...
    infoToast(
      `${name.replace(/([A-Z])/g, " $1").toLowerCase()} ${checked ? "enabled" : "disabled"}`,
    );
  };
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        localStorage.setItem(
          "user",
          JSON.stringify({ ...storedUser, profileImage: updatedProfileImage }),
        );
        successToast("Profile picture updated!");
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem(
          "user",
          JSON.stringify({ ...storedUser, profileImage: emptyImage }),
        );
        successToast("Profile picture deleted!");
  ...
  const getCurrentLocation = () => {
    ...
    } else warningToast("Geolocation not supported");
  };
  ...
  return (
    <div className="mobpro-mobile-profile-container">
      <div className="mobpro-mobile-profile-header">
        <button
          className="mobpro-mobile-back-btn"
        ...
        <div className="mobpro-mobile-header-actions">
          {!isEditing && !viewMode && (
            <button
              className="mobpro-mobile-edit-btn"
          ...
          {isEditing && (
            <>
              <button
                className="mobpro-mobile-cancel-btn"
                onClick={handleEditCancel}
              >
                <FaTimes />
              </button>
              <button
                className="mobpro-mobile-save-btn"
                onClick={handleSave}
                disabled={isLoading}
              >
                <FaSave />
