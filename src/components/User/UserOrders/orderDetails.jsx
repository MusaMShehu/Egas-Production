...
const OrderDetails = () => {
  ...
  const handleReorder = async () => {
    if (!order) return;
    infoToast(`Reordering items from order #${order.orderId || order._id}...`);
    try {
      const response = await ApiService.orders.reorder(order._id);
      if (response.success) {
      ...
      errorToast(err.message || "Failed to reorder. Please try again.");
    }
  };
    ...
    <div className="order-details-container">
      ...
      <section className="order-section">
        ...
        <div className="info-grid">
          <div className="info-item">
            <strong>Address:</strong> 
            <span>{order.shippingAddress?.address || order.address || "Not specified"}</span>
          </div>
          <div className="info-item">
            <strong>Street:</strong> 
            <span>{order.shippingAddress?.street || "Not specified"}</span>
          ...
          <div className="info-item">
            <strong>City:</strong>
            <span>{order.shippingAddress?.city || order.city || "Not specified"}</span>
          </div>
          <div className="info-item">
            <strong>State:</strong> 
            <span>{order.shippingAddress?.state || order.state || "Not specified"}</span>
          </div>
        </div>
      </section>
      ...
      <section className="order-section">
        ...
        <div className="order-items">
          {(order.items || order.products || []).map((item, index) => (
            <div key={index} className="order-item">
              <img
                src={item.image || item.product?.image || "/images/default-product.jpg"}
                alt={item.name || item.productName || item.product?.name || "Product"}
                onError={(e) => handleImageError(e, item.name || item.productName)}
              />
              <div className="item-info">
                <p className="item-name"><strong>{item.name || item.productName || item.product?.name || "Product"}</strong></p>
                <p className="item-quantity">Quantity: {item.quantity}</p>
                <p className="item-price">Price: {formatCurrency(item.price)}</p>
              </div>
              <div className="item-total">
      ...
      <section className="order-section totals">
        ...
        <div className="summary-grid">
          <div className="summary-item">
            <strong>Subtotal:</strong> 
            <span>{formatCurrency(order.items?.reduce((acc, i) => acc + (i.price || 0) * (i.quantity || 1), 0) || order.subtotal || 0)}</span>
          </div>
          <div className="summary-item">
            <strong>Shipping:</strong> 
            <span>{formatCurrency(order.shippingPrice || order.deliveryFee || 0)}</span>
          </div>
          <div className="summary-item">
            <strong>Tax:</strong> 
            <span>{formatCurrency(order.taxPrice || 0)}</span>
