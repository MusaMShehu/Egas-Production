...
// src/pages/MobileUserHistory.jsx
import React, { useState, useEffect } from "react";
import {
  FaTimes,
  FaFilter,
...
const History = () => {
      ...
      if (data && data.success) {
        ...
        setPagination((prev) => ({
          ...
          [tab]: {
            ...
            total: data.total || 0,
            pages:
              data.pages || Math.ceil((data.total || 0) / limit) || 1,
          },
        }));
        ...
        if (searchQuery || filter !== "all") {
          ...
          if (resultCount > 0)
            successToast(`Found ${resultCount} ${tab} matching your criteria`);
          else infoToast(`No ${tab} found matching your criteria`);
        } else if (page > 1) infoToast(`Loaded page ${page} of ${tab}`);
  ...
  useEffect(() => {
    if (!loadedData[activeTab]) loadActiveTabData(1);
  }, [activeTab]);
  useEffect(() => {
    if (loadedData[activeTab]) loadActiveTabData(1);
  }, [filter, searchQuery]);
  ...
  const formatCurrency = (amount) =>
    `₦${(typeof amount === "number" ? amount : parseFloat(amount) || 0).toLocaleString()}`;
  ...
  const getStatusClass = (status) => {
    ...
    if (activeTab === "subscriptions") {
      switch (statusLower) {
        case "active":
          return "mobhis-status-active";
        case "paused":
          return "mobhis-status-paused";
        case "pending":
          return "mobhis-status-pending";
        case "expired":
          return "mobhis-status-expired";
        case "cancelled":
          return "mobhis-status-cancelled";
        default:
          return "mobhis-status-pending";
      }
    }
    if (activeTab === "orders") {
      switch (statusLower) {
        case "processing":
          return "mobhis-status-processing";
        case "in-transit":
        case "in_transit":
        case "intransit":
          return "mobhis-status-in-transit";
        case "delivered":
          return "mobhis-status-delivered";
        case "cancelled":
          return "mobhis-status-cancelled";
        default:
          return "mobhis-status-processing";
    ...
    if (activeTab === "payments") {
      switch (statusLower) {
        case "pending":
          return "mobhis-status-pending";
        case "completed":
          return "mobhis-status-completed";
        case "failed":
          return "mobhis-status-failed";
        default:
          return "mobhis-status-pending";
      }
    }
  ...
  return (
    <div className="mobhis-history-page">
      ...
      <div className="mobhis-desktop-header">
        ...
        <div className="mobhis-desktop-header-actions">
          <form
            ...
            className="mobhis-desktop-search-wrapper"
          >
            <div className="mobhis-desktop-search-input-wrapper">
              <FaSearch className="mobhis-search-icon" />
              <input
      ...
      {showFilters && (
        <div className="mobhis-mobile-filter-panel">
          ...
          <div className="mobhis-filter-panel-content">
            ...
            {filter !== "all" && (
              <button
                className="mobhis-mobile-clear-filters"
                onClick={clearFilters}
              >
                Clear Filters
              </button>
              ...
              {filteredData.map((item) => (
                <div key={item._id || item.id} className="mobhis-mobile-card">
                  <div className="mobhis-mobile-card-header">
                    <div className="mobhis-mobile-card-left">
                      <div className="mobhis-mobile-card-title">
                        {activeTab === "orders" && (
                          <>
                            <div className="mobhis-mobile-card-id">
                              Order #
                              {item.orderId?.slice(-8) || item._id?.slice(-8)}
                            </div>
                            <div className="mobhis-mobile-card-date">
                        ...
                        {activeTab === "subscriptions" && (
                          <>
                            <div className="mobhis-mobile-card-plan">
                              {item.planName}
                            </div>
                            <div className="mobhis-mobile-card-size">
                              {item.size} • {item.frequency}
                            </div>
                          </>
                        )}
                        {activeTab === "payments" && (
                    ...
                    <div className="mobhis-mobile-card-right">
                      <span
                        className={`mobhis-mobile-status ${getStatusClass(item.status)}`}
                      >
                        {getStatusIcon(item.status)}
                        <span>{getStatusText(item.status)}</span>
                      </span>
                  ...
                  <div className="mobhis-mobile-card-body">
                    {activeTab === "orders" && (
                      <div className="mobhis-mobile-card-details">
                        <div className="mobhis-mobile-card-amount">
                        ...
                        <div className="mobhis-mobile-card-items">
                          ...
                          {(!item.items || item.items.length === 0) && (
                            <span className="mobhis-mobile-card-item">
                              No items
                            </span>
                          )}
                        </div>
                    ...
                    {activeTab === "payments" && (
                      <div className="mobhis-mobile-card-details">
                        ...
                        <div className="mobhis-mobile-card-method">
                          {getMethodName(item.method)}
                        </div>
                        {item.failureReason && (
                          <div className="mobhis-mobile-card-reason">
                            {item.failureReason}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="mobhis-mobile-card-footer">
                    <button
                      ...
                      onClick={() => viewItemDetails(item)}
                    >
                      <FaEye className="mobhis-btn-icon" /> View Details
                    </button>
                    {activeTab === "orders" && item.status === "delivered" && (
                      <button
                        className="mobhis-mobile-action-btn"
                        onClick={() => handleReorder(item)}
                      >
                        <FaRedo className="mobhis-btn-icon" /> Reorder
                      </button>
                    )}
                    {activeTab === "payments" && item.status === "failed" && (
                      <button
                        ...
                        onClick={() => handleRetryPayment(item)}
                      >
                        <FaRedo className="mobhis-btn-icon" /> Retry
                      </button>
                    )}
                  </div>
                </div>
                        ...
                        <>
                          ...
                          <td data-label="Actions">
                            <button
                              ...
                              onClick={() => viewItemDetails(item)}
                            >
                              <FaEye className="mobhis-btn-icon" /> View
                            </button>
                          </td>
                        </>
                      )}
                      ...
                      {activeTab === "payments" && (
                        <>
                          ...
                          >
                            ...
                            {item.failureReason && (
                              <div className="mobhis-failure-reason">
                                ({item.failureReason})
                              </div>
                            )}
                          </td>
                ...
                <div className="mobhis-desktop-pagination-numbers">
                  {getPageNumbers().map((pageNum, index) =>
                    pageNum === "..." ? (
                      <span
                        key={`ellipsis-${index}`}
                        className="mobhis-desktop-pagination-ellipsis"
                      >
                        ...
                      </span>
                    ) : (
                      <button
                        key={pageNum}
                        className={`mobhis-desktop-pagination-number ${currentPagination.page === pageNum ? "active" : ""}`}
                        onClick={() => handlePageChange(pageNum)}
                      >
                        {pageNum}
                      </button>
      ...
      {activeItem && (
        <div className="mobhis-mobile-detail-modal">
          <div
            className="mobhis-mobile-modal-overlay"
          ...
          <div className="mobhis-mobile-modal-content">
            <div className="mobhis-mobile-modal-header">
              ...
              <button
                className="mobhis-mobile-close-btn"
                onClick={closeItemDetails}
              >
                <FaTimes />
              </button>
            ...
            <div className="mobhis-mobile-modal-body">
              <div className="mobhis-mobile-modal-details">
                {activeTab === "orders" && (
                  <>
                    <div className="mobhis-mobile-detail-row">
                      <span className="mobhis-mobile-detail-label">
                        Order ID:
                      </span>
                      <span className="mobhis-mobile-detail-value">
                    ...
                    <div className="mobhis-mobile-detail-row">
                      ...
                      <span
                        className={`mobhis-mobile-detail-value ${getStatusClass(activeItem.status)}`}
                      >
                    ...
                    <div className="mobhis-mobile-detail-row">
                      ...
                      <span className="mobhis-mobile-detail-value mobhis-amount">
                    ...
                    <div className="mobhis-mobile-detail-section">
                      ...
                      {activeItem.items?.map((item, index) => (
                        <div key={index} className="mobhis-mobile-order-item">
                          <div className="mobhis-mobile-item-name">
                            {item.name}
                          </div>
                          <div className="mobhis-mobile-item-details">
                  ...
                  <>
                    ...
                    {activeItem.failureReason && (
                      <div className="mobhis-mobile-detail-row">
                        ...
                        <span className="mobhis-mobile-detail-value mobhis-failure-reason">
                          {activeItem.failureReason}
                        </span>
                      </div>
                    )}
            ...
            <div className="mobhis-mobile-modal-footer">
              <button
                className="mobhis-mobile-btn-close"
                onClick={closeItemDetails}
              >
                Close
              </button>
              ...
              {activeTab === "payments" && activeItem.status === "failed" && (
                <button
                  ...
                  onClick={() => handleRetryPayment(activeItem)}
                >
                  <FaRedo className="mobhis-btn-icon" /> Retry Payment
                </button>
              )}
            </div>
          </div>
      ...
      {activeItem && (
        <div className="mobhis-desktop-detail-modal">
          <div
            className="mobhis-desktop-modal-overlay"
            onClick={closeItemDetails}
          ></div>
          <div className="mobhis-desktop-modal-content">
            <div className="mobhis-desktop-modal-header">
              ...
              <button
                className="mobhis-desktop-close-btn"
                onClick={closeItemDetails}
              >
                <FaTimes />
              </button>
              ...
              <div className="mobhis-desktop-modal-details">
                {activeTab === "orders" && (
                  <>
                    <div className="mobhis-detail-row">
                      <span className="mobhis-detail-label">Order ID:</span>
                      <span className="mobhis-detail-value">
                        {activeItem.orderId || activeItem._id}
                      </span>
                    </div>
                    <div className="mobhis-detail-row">
                    ...
                    <div className="mobhis-detail-row">
                      <span className="mobhis-detail-label">Status:</span>
                      <span
                        className={`mobhis-detail-value ${getStatusClass(activeItem.status)}`}
                      >
                        {getStatusIcon(activeItem.status)}{" "}
                        {getStatusText(activeItem.status)}
                      </span>
                    ...
                    <div className="mobhis-detail-row">
                      <span className="mobhis-detail-label">Total Amount:</span>
                      <span className="mobhis-detail-value mobhis-amount">
                        {formatCurrency(activeItem.totalAmount)}
                      </span>
                    </div>
                    <div className="mobhis-detail-section">
                ...
                {activeTab === "payments" && (
                  <>
                    <div className="mobhis-detail-row">
                      <span className="mobhis-detail-label">
                        Transaction ID:
                      </span>
                      <span className="mobhis-detail-value">
                        {activeItem.transactionId || activeItem._id}
