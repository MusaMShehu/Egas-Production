...
const Subscriptions = () => {
  ...
  // Fetch all user data including subscriptions and wallet balance
  const fetchUserData = async () => {
    setIsLoading(true);
    infoToast('Loading your subscriptions...');
    ...
    try {
      ...
      try {
        ...
        // Non-critical error, continue with subscription fetch
      }

      // Fetch subscriptions using the API service
      const subscriptionsData = await subscriptionApi.getMySubscriptions();
      const subscriptionsList = subscriptionsData.data || [];
      
      // Enhance subscriptions with calculated next delivery dates
      const enhancedSubscriptions = subscriptionsList.map(sub => ({
        ...sub,
        nextDeliveryDate: subscriptionApi.calculateNextDeliveryDate(sub) || sub.nextDeliveryDate,
        // Ensure dates exist
      ...
      if (enhancedSubscriptions.length === 0) {
        ...
        successToast(`Loaded ${enhancedSubscriptions.length} subscriptions successfully`);
      }
  ...
  const handlePausePlan = async (subscriptionId) => {
    ...
    try {
      const result = await subscriptionApi.pauseSubscription(subscriptionId);
        ...
        setSubscriptions(prev =>
          prev.map(sub =>
            sub._id === subscriptionId ? { 
              ...
              ...result.data,
              status: 'paused',
              nextDeliveryDate: subscriptionApi.calculateNextDeliveryDate({ ...sub, status: 'paused' })
            } : sub
          )
        );
        successToast('Subscription paused successfully');
  ...
  const handleResumePlan = async (subscriptionId) => {
    ...
    try {
      const result = await subscriptionApi.resumeSubscription(subscriptionId);
      
      if (result.success || result.data) {
        const updatedSub = { ...subscription, ...result.data, status: 'active' };
        updatedSub.nextDeliveryDate = subscriptionApi.calculateNextDeliveryDate(updatedSub);
        
        setSubscriptions(prev =>
          prev.map(sub =>
            sub._id === subscriptionId ? updatedSub : sub
          )
        );
        successToast('Subscription resumed successfully');
        notificationManager.subscriptionResumed(subscriptionId, user?.id);
