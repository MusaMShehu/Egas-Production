...
// src/pages/MobileUserPayment.jsx
...
const Payments = () => {
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      } else throw new Error(data.message || "Top-up failed");
    } catch (error) {
      console.error("Error processing top-up:", error);
  ...
  const filteredActivities =
    paymentData.recentActivities?.filter((activity) => {
      const matchesSearch =
        activity.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        activity.type?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        activity.status?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    infoToast(
      `Showing ${tab === "all" ? "all transactions" : tab + " transactions"}`,
    );
  };
  const handleSearchChange = (e) => {
    setSearchTerm(e.target.value);
    if (e.target.value) infoToast(`Searching for "${e.target.value}"...`);
  };
  const handleClearSearch = () => {
    setSearchTerm("");
    successToast("Search cleared");
    ...
    <div className="mobpay-payments-page">
      <div className="mobpay-mobile-header">
        ...
        <div className="mobpay-mobile-quick-stats">
          <div className="mobpay-mobile-stat">
            ...
            <div className="mobpay-mobile-stat-info">
              <div className="mobpay-mobile-stat-label">Wallet</div>
              <div className="mobpay-mobile-stat-value">
      ...
      <div className="mobpay-desktop-header">
        ...
        <div className="mobpay-desktop-header-actions">
          ...
          <button
            ...
            disabled={isProcessing}
          >
            <FaPlus className="mobpay-btn-icon" /> Top Up Wallet
          </button>
        </div>
      </div>
      {error && (
      ...
      <div className="mobpay-mobile-stats">
        <div className="mobpay-mobile-stat-card">
          ...
          <div className="mobpay-mobile-stat-card-content">
            <div className="mobpay-mobile-stat-card-label">This Month</div>
            <div className="mobpay-mobile-stat-card-value">
              {formatCurrency(paymentData.thisMonthSpent)}
            </div>
            <div className="mobpay-mobile-stat-card-breakdown">
              <span>Orders: {formatCurrency(paymentData.orderMonthly)}</span>
              <span>
                Subs: {formatCurrency(paymentData.subscriptionMonthly)}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="mobpay-desktop-stats-container">
        <div className="mobpay-desktop-stat-card mobpay-balance-card">
          ...
          <h3>Wallet Balance</h3>
          <div className="mobpay-desktop-value">
            {formatCurrency(paymentData.walletBalance)}
          </div>
          <div className="mobpay-desktop-stat-note">
        ...
        <div className="mobpay-desktop-stat-card mobpay-spending-card">
          ...
          <div className="mobpay-desktop-stat-breakdown">
            <span>Orders: {formatCurrency(paymentData.orderMonthly)}</span>
            <span>
              Subscriptions: {formatCurrency(paymentData.subscriptionMonthly)}
            </span>
          </div>
        </div>
        <div className="mobpay-desktop-stat-card mobpay-spending-card">
          ...
          <div className="mobpay-desktop-stat-breakdown">
            <span>Orders: {formatCurrency(paymentData.orderTotal)}</span>
            <span>
              Subscriptions: {formatCurrency(paymentData.subscriptionTotal)}
            </span>
          </div>
        </div>
        <div className="mobpay-desktop-stat-card mobpay-topup-card">
          ...
          <h3>Total Top-ups</h3>
          <div className="mobpay-desktop-value">
            {formatCurrency(paymentData.topupTotal)}
          </div>
          <div className="mobpay-desktop-stat-breakdown">
            <span>This month: {formatCurrency(paymentData.topupMonthly)}</span>
          </div>
      ...
      <div className="mobpay-content-section">
        <div className="mobpay-desktop-section-header">
          ...
          <span className="mobpay-desktop-count-badge">
            {sortedActivities.length} transactions
          </span>
        </div>
        <div className="mobpay-mobile-section-header">
          <div className="mobpay-mobile-section-title">
            <h2>Transactions</h2>
            <span className="mobpay-mobile-count">
        ...
        {sortedActivities.length > 0 ? (
          <>
            <div className="mobpay-mobile-list">
              {sortedActivities.map((activity, index) => (
                <div
                  key={`${activity.type}-${index}-${activity.createdAt}`}
                  className="mobpay-mobile-card"
                >
                  <div className="mobpay-mobile-card-header">
                    <div className="mobpay-mobile-card-left">
                      <div className="mobpay-mobile-activity-type">
                    ...
                    <div className="mobpay-mobile-card-right">
                      <div
                        className={`mobpay-mobile-status-badge mobpay-mobile-status-${getStatusColor(activity.status, activity.type)}`}
                      >
                        {activity.status || "pending"}
                      </div>
                    </div>
                  ...
                  <div className="mobpay-mobile-card-body">
                    ...
                    <div className="mobpay-mobile-card-footer">
                      <div
                        className={`mobpay-mobile-amount ${activity.type === "wallet_transaction" && activity.status?.toLowerCase() === "credit" ? "credit" : ""}`}
                      >
                        {activity.type === "wallet_transaction" &&
                        activity.status?.toLowerCase() === "credit"
                          ? "+"
                          : ""}
                        {formatCurrency(activity.amount)}
                      </div>
                    ...
                    {sortedActivities.map((activity, index) => (
                      ...
                      >
                        ...
                        <td
                          ...
                          className="mobpay-desktop-activity-title"
                        >
                          {activity.title || "N/A"}
                        </td>
                        <td data-label="Date">
                        ...
                        <td
                          data-label="Amount"
                          className="mobpay-desktop-amount"
                        >
                          {activity.type === "wallet_transaction" &&
                          activity.status?.toLowerCase() === "credit"
                            ? "+"
                        ...
                        <td data-label="Status">
                          <span
                            className={`mobpay-desktop-status-badge mobpay-desktop-status-${getStatusColor(activity.status, activity.type)}`}
                          >
                            {activity.status || "pending"}
                          </span>
                        </td>
