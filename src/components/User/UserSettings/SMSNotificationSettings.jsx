...
// src/pages/SMSNotificationSettings.jsx
import React, { useState, useEffect } from 'react';
import {
  Box,
  Card,
...
const SMSNotificationSettings = () => {
  ...
  const fetchSettings = async () => {
    try {
      setLoading(true);
      infoToast('Loading notification settings...');
      // Fetch settings from the notifications endpoint
      const response = await ApiService.notifications.getSettings?.() || await ApiService.settings.get();
      if (response && response.smsSettings) {
        setSettings(response.smsSettings);
      } else if (response && response.data?.smsSettings) {
        setSettings(response.data.smsSettings);
      }
      successToast('Settings loaded successfully');
    } catch (error) {
      console.error('Failed to fetch settings:', error);
      errorToast('Failed to load notification settings');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggle = (setting) => (event) => {
    ...
    infoToast(`${setting.replace(/([A-Z])/g, ' $1').toLowerCase()} ${event.target.checked ? 'enabled' : 'disabled'}`);
  };

  const handleSave = async () => {
    try {
      setSaveLoading(true);
      infoToast('Saving notification settings...');
      // Save settings to the notifications endpoint
      await ApiService.notifications.updateSettings?.({ smsSettings: settings }) || 
      await ApiService.settings.update({ smsSettings: settings });
      showNotification('Settings saved successfully!', 'success');
      successToast('Notification preferences updated');
    } catch (error) {
      showNotification('Failed to save settings', 'error');
