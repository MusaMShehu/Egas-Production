...
// src/components/subscriptionPayment.js
...
const SubscriptionPayment = ({ plan, onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await ApiService.payments.initializeSubscriptionPayment(paymentData);
    ...
    <div className="payment-container">
      <div className="payment-card">
        ...
        <div className="plan-details">
          ...
          <div className="frequency">{plan.frequency}</div>
          {plan.features && (<ul className="features-list">{plan.features.map((feature, index) => (<li key={index}>{feature}</li>))}</ul>)}
        </div>

        <form onSubmit={handleSubmit} className="payment-form">
          <div className="form-group">
            <label htmlFor="email">Email Address</label>
            <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} required disabled={isProcessing} />
          </div>
          ...
          <div className="payment-actions">
            ...
            <button type="submit" className="btn btn-primary" disabled={isProcessing}>{isProcessing ? 'Processing...' : `Pay ₦${plan.price.toLocaleString()}`}</button>
          </div>
        </form>
