...
//   // Price calculation helper - UPDATED to match backend
//   const calculatePrice = (plan, size, frequency, subscriptionPeriod = 1) => {
//     if (!plan) return 0;
...
//       // One-time/emergency: Only 1 delivery
//       totalDeliveries = 1;
//     } else {
...
//   // Helper function for price breakdown
...
//   const confirmPlan = (planData) => {
//     const plan = plans.find(p => p._id === planData.planId);
//     const priceBreakdown = getPriceBreakdown(
//       plan,
//       planData.size,
//       planData.frequency,
//       planData.subscriptionPeriod
//     );
...
//         {/* Plan Price with breakdown tooltip */}
//         <div className="subplan-plan-price-display">
//           <span className="subplan-price-amount">{formatCurrency(price)}</span>
//           {plan.type !== 'one-time' && plan.type !== 'emergency' && priceBreakdown && (
//             <div className="subplan-price-breakdown-tooltip">
//               <FaInfoCircle className="subplan-info-icon" />
//               <div className="subplan-price-breakdown">
//                 <strong>Price Breakdown:</strong>
//                 <div>Size: {currentSize} × ₦{priceBreakdown.pricePerKg?.toLocaleString()}/kg</div>
//                 <div>Base amount: ₦{priceBreakdown.baseAmount?.toLocaleString()}</div>
...
//         {plan.type !== 'one-time' && plan.type !== 'emergency' && priceBreakdown && (
//           <div className="subplan-delivery-info">
//             <div className="subplan-delivery-count">
//               <FaTruck className="subplan-info-icon" />
//               <span>{priceBreakdown.breakdownShort}</span>
//             </div>
//             {priceBreakdown.hasInitialDelivery && (
//               <div className="subplan-initial-note">
//                 Includes 1 initial delivery on day 1
//               </div>
//             )}
//           </div>
...
//                 <p><strong>Subscription Period:</strong> {selectedPlan.subscriptionPeriod} month{selectedPlan.subscriptionPeriod > 1 ? 's' : ''}</p>
//               )}
//               <p className="subplan-total-price"><strong>Total Price:</strong> {formatCurrency(selectedPlan.price)}</p>

//               {/* Price calculation breakdown */}
//               {selectedPlan.priceBreakdown && (
//                 <div className="subplan-price-calculation">
//                   <p style={{ fontSize: '0.85rem', color: '#7f8c8d', marginTop: '8px' }}>
//                     Calculation: {selectedPlan.size} × ₦{selectedPlan.priceBreakdown.pricePerKg?.toLocaleString()}/kg × {selectedPlan.priceBreakdown.totalDeliveries} deliveries
//                   </p>
//                 </div>
//               )}
//             </div>
...
//                 disabled={isProcessing || (paymentMethod === "wallet" && walletBalance < selectedPlan.price)}
//                 className="subplan-confirm-payment-btn"
//                 style={{ backgroundColor: getPlanColor(selectedPlan.planType) }}
//               >
//                 {isProcessing
//                   ? "Processing..."
//                   : paymentMethod === "wallet"
...
const SubscriptionPlans = () => {
    ...
    if (frequency === "One-Time" || frequency === "Emergency") {
      ...
      if (subscriptionPeriod === 1) {
        ...
        totalDeliveries =
          deliveriesPerMonth +
          1 +
          deliveriesPerMonth * (subscriptionPeriod - 1);
      }
    }
  ...
  const getPriceBreakdown = (plan, size, frequency, subscriptionPeriod = 1) => {
    ...
    return {
      ...
      deliveriesPerMonth,
      hasInitialDelivery: !(
        frequency === "One-Time" || frequency === "Emergency"
      ),
    };
  };
  ...
  const confirmPlan = (planData) => {
    ...
    setSelectedPlan({ ...planData, priceBreakdown });
    setShowSummary(true);
  };
        ...
        <div className="subplan-plan-header">
          <div className="subplan-plan-title-section">
            ...
            <div>
              <h3>{plan.name}</h3>
              <span
                className="subplan-plan-type-badge"
                style={{ backgroundColor: getPlanColor(plan.type) }}
              >
                {plan.type === "one-time"
                  ? "Single Purchase"
            ...
            priceBreakdown && (
              <div className="subplan-price-breakdown-tooltip">
                ...
                <div className="subplan-price-breakdown">
                  ...
                  <div>
                    Size: {currentSize} × ₦
                    {priceBreakdown.pricePerKg?.toLocaleString()}/kg
                  </div>
                  <div>
                    Base amount: ₦{priceBreakdown.baseAmount?.toLocaleString()}
                  </div>
                  <div>Deliveries: {priceBreakdown.breakdownShort}</div>
                  <div>Total: ₦{price.toLocaleString()}</div>
                </div>
        ...
        <div className="subplan-plan-options">
          ...
          plan.type === "emergency" ? (
            ...
            <div className="subplan-option-info">
              <FaGasPump />
              <span>
                Cylinder Size: <strong>{plan.baseSize}</strong>
              </span>
            </div>
          )}
          {(plan.type === "custom" || plan.type === "preset") && (
      ...
      <ul className="subplan-plan-features-list">
        {features?.map((feature, index) => (
          <li
            ...
            className={
              feature.included
                ? "subplan-feature-included"
                : "subplan-feature-excluded"
            }
          >
            {feature.included ? (
              <FaCheck className="subplan-feature-icon" />
            ) : (
              <FaTimes className="subplan-feature-icon" />
            )}
            <span>
  ...
  return (
    <div className="subplan-plans-container">
        ...
        <div className="subplan-category-section">
          <div className="subplan-category-header">
            ...
            <div>
              <h2>Emergency Delivery</h2>
              <p className="subplan-category-description">
                Need gas urgently? Get immediate delivery within hours. Perfect
                for unexpected situations when you run out of gas.
              </p>
            </div>
          </div>
          <div className="subplan-plans-grid">
        ...
        <div className="subplan-preset-category-section">
          <div className="subplan-category-header">
            ...
            <div>
              <h2>Regular Subscription Plans</h2>
              <p className="subplan-category-description">
                Choose from our flexible subscription plans for regular gas
                delivery. Save money with automatic deliveries based on your
                usage pattern.
              </p>
            </div>
          </div>
        ...
        <div className="subplan-category-section">
          <div className="subplan-category-header">
            ...
            <div>
              <h2>Build Your Own Plan</h2>
              <p className="subplan-category-description">
                Customize every aspect of your gas delivery. Choose your
                cylinder size, delivery frequency, and subscription period.
              </p>
            </div>
          </div>
          <div className="subplan-plans-grid">
      ...
      {planDetailsModal && (
        <div className="subplan-plan-details-modal">
          <div className="subplan-modal-content">
            <div className="subplan-modal-header">
              <div className="subplan-modal-title">
                ...
                <div>
                  <h2>{planDetailsModal.name}</h2>
                  <span
                    className="subplan-modal-plan-type"
                    style={{
                      backgroundColor: getPlanColor(planDetailsModal.type),
                    }}
                  >
                    {planDetailsModal.type === "one-time"
                      ? "One-Time Purchase"
                      : planDetailsModal.type === "emergency"
                        ? "Emergency Delivery"
                        : planDetailsModal.type === "custom"
                          ? "Custom Plan"
                          : "Subscription Plan"}
                  </span>
                </div>
              ...
              <div className="subplan-plan-specifications">
                ...
                <div className="subplan-spec-grid">
                  <div className="subplan-spec-item">
                    <span className="subplan-spec-label">Base Size:</span>
                    <span className="subplan-spec-value">
                      {planDetailsModal.baseSize}
                    </span>
                  </div>
                  <div className="subplan-spec-item">
                    <span className="subplan-spec-label">Price per Kg:</span>
                    <span className="subplan-spec-value">
                      {formatCurrency(planDetailsModal.pricePerKg)}
                    </span>
                  </div>
                  {planDetailsModal.deliveryFrequency && (
                    <div className="subplan-spec-item">
                      <span className="subplan-spec-label">
                        Delivery Frequency:
                      </span>
                      <span className="subplan-spec-value">
                        {planDetailsModal.deliveryFrequency.join(", ")}
                      </span>
                    </div>
                  )}
                  {planDetailsModal.subscriptionPeriod && (
                  ...
                  {planDetailsModal.cylinderSizes && (
                    <div className="subplan-spec-item">
                      ...
                      <span className="subplan-spec-value">
                        {planDetailsModal.cylinderSizes.join(", ")}
                      </span>
                    </div>
                  )}
                  {planDetailsModal.deliveryInfo && (
                    <>
                      <div className="subplan-spec-item">
                        ...
                        <span className="subplan-spec-value">
                          {planDetailsModal.deliveryInfo.deliveryTime}
                        </span>
                      </div>
                      <div className="subplan-spec-item">
                        <span className="subplan-spec-label">
                        ...
                        <span className="subplan-spec-value">
                          {planDetailsModal.deliveryInfo.freeDelivery
                            ? "Yes"
                            : "No"}
                        </span>
                      </div>
            ...
            <div className="subplan-modal-footer">
              <button
                className="subplan-btn-secondary"
                onClick={() => setPlanDetailsModal(null)}
              >
                Close
              </button>
              <button
                ...
                onClick={() => {
                  setPlanDetailsModal(null);
                  const currentSize =
                    selectedSize[planDetailsModal._id] ||
                    planDetailsModal.baseSize;
                  const currentFrequency =
                    selectedFrequencies[planDetailsModal._id] || "Monthly";
                  const currentPeriod =
                    selectedSubscriptionPeriods[planDetailsModal._id] || 1;
                  const price = calculatePrice(
                    planDetailsModal,
                    currentSize,
                    currentFrequency,
        ...
        <div className="subplan-summary-modal">
          <div className="subplan-summary-card">
            ...
            <div className="subplan-order-details">
              ...
              <p className="subplan-total-price">
              ...
              {selectedPlan.priceBreakdown && (
                <div className="subplan-price-calculation">
                  ...
                  >
                    Calculation: {selectedPlan.size} × ₦
                    {selectedPlan.priceBreakdown.pricePerKg?.toLocaleString()}
                    /kg × {selectedPlan.priceBreakdown.totalDeliveries}{" "}
                    deliveries
                  </p>
                </div>
              )}
            ...
            <div className="subplan-payment-method">
              ...
              <div className="subplan-payment-options">
                <label
                  className={`subplan-payment-option ${paymentMethod === "paystack" ? "subplan-selected" : ""}`}
                >
                  <input
                    className="subplan-payment-option-input"
                    type="radio"
                    name="paymentMethod"
                    value="paystack"
                    checked={paymentMethod === "paystack"}
                ...
                <label
                  className={`subplan-payment-option ${paymentMethod === "wallet" ? "subplan-selected" : ""}`}
                >
                  <input
                    className="subplan-payment-option-input"
                    type="radio"
                  ...
                  <FaWallet />
                  <span>Pay From Wallet Balance</span>
                  <span className="subplan-wallet-balance">
      ...
      {isProcessing && (
        <div className="subplan-loading-overlay">
          <div className="subplan-loading-spinner">Initializing Payment...</div>
        </div>
      )}
    </div>
  );
