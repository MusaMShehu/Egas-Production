...
// src/pages/MobileSubscriptionPlans.jsx
import React, { useState, useEffect, useCallback, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
...
import './MobileSubscriptionPlans.css';
import { 
  FaWallet, FaCreditCard, FaFire, FaBolt, FaGasPump, 
  FaCalendar, FaCheck, FaTimes, FaEye, FaTag,
  FaUserFriends, FaExclamationTriangle, FaArrowRight,
  FaInfoCircle, FaTruck, FaShieldAlt
} from 'react-icons/fa';
import { successToast, errorToast, infoToast, warningToast } from "../../utils/toast";
...
const MobileSubscriptionPlans = () => {
  ...
  const fetchSubscriptionPlans = useCallback(async () => {
    try {
      ...
      if (data?.success) {
        ...
        // Sort plans: one-time first, then emergency, then custom, then preset
        const sortedPlans = allPlans.sort((a, b) => {
          if (a.type === 'one-time' && b.type !== 'one-time') return -1;
          if (a.type !== 'one-time' && b.type === 'one-time') return 1;
          if (a.type === 'emergency' && b.type !== 'emergency') return -1;
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [isAuthenticated, fetchSubscriptionPlans, fetchWalletBalance]);
  ...
  const calculatePrice = (plan, size, frequency, subscriptionPeriod = 1) => {
    ...
    switch (frequency) {
      ...
      case 'Bi-weekly': deliveriesPerMonth = 2; break;
      case 'Monthly': deliveriesPerMonth = 1; break;
      case 'One-Time': case 'Emergency': deliveriesPerMonth = 1; break;
      default: deliveriesPerMonth = 1;
    }
  ...
  const getPriceBreakdown = (plan, size, frequency, subscriptionPeriod = 1) => {
    ...
    return {
      totalPrice: Math.round(baseAmount * totalDeliveries),
      breakdown, breakdownShort, pricePerKg,
      baseAmount: Math.round(baseAmount), totalDeliveries,
      deliveriesPerMonth, hasInitialDelivery: !(frequency === 'One-Time' || frequency === 'Emergency')
    };
  };

  const getPlanIcon = (planType) => {
    switch (planType) {
      case 'one-time': return <FaTag className="mobsubplan-plan-icon mobsubplan-one-time" />;
      case 'emergency': return <FaBolt className="mobsubplan-plan-icon mobsubplan-emergency" />;
      case 'custom': return <FaGasPump className="mobsubplan-plan-icon mobsubplan-custom" />;
      default: return <FaFire className="mobsubplan-plan-icon mobsubplan-default" />;
    }
  };

  const getPlanColor = (planType) => {
    switch (planType) {
      case 'one-time': return '#f39c12';
  ...
  const getSizeOptions = (plan) => {
    switch (plan.type) {
      case 'custom':
        ...
        const max = plan.cylinderSizeRange?.max || 100;
        return Array.from({ length: max - min + 1 }, (_, i) => `${i + min}kg`);
      case 'one-time': return (plan.cylinderSizes || []).map((size) => `${size}kg`);
      case 'emergency': return plan.cylinderSizes || ['6kg', '12kg', '25kg', '50kg'];
      default: return [plan.baseSize];
    }
  };
  ...
  const getFrequencyOptions = (plan) => {
    switch (plan.type) {
      ...
      case 'preset': return plan.deliveryFrequency || ['Monthly'];
  ...
  const handleViewDetails = (plan) => setPlanDetailsModal(plan);
  ...
  const initializePayment = async (planData) => {
    ...
    try {
      ...
      const response = await ApiService.subscriptions.create(requestBody);
      ...
      if (error.message?.includes('Insufficient wallet balance')) {
        ...
        errorToast(error.message || 'Failed to process payment. Please try again.');
      }
    } finally {
      setIsProcessing(false);
      setShowSummary(false);
    ...
    return (
      <div className="mobsubplan-mobile-subscription-plans mobsubplan-error">
        <div className="mobsubplan-error-content">
          ...
          <h2>Unable to Load Plans</h2>
          <p>{error}</p>
          <button className="mobsubplan-retry-btn" onClick={handleRetry}>Try Again</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="mobsubplan-mobile-subscription-plans">
        <div className="mobsubplan-login-prompt">
          <h1>Choose a Subscription Plan</h1>
          <p>Please log in to view and subscribe to our plans</p>
          <button className="mobsubplan-login-btn" onClick={() => navigate('/auth')}>Log In</button>
        </div>
      </div>
    );
  }
  ...
  const renderPlanCard = (plan) => {
    ...
    return (
      <div key={plan._id} className="mobsubplan-plan-card" style={{ borderLeftColor: getPlanColor(plan.type) }}>
        <div className="mobsubplan-plan-card-header">
          <div className="mobsubplan-plan-title-section">
            {getPlanIcon(plan.type)}
            <div className="mobsubplan-plan-title-content">
          ...
          <button className="mobsubplan-view-details-btn" onClick={() => handleViewDetails(plan)}>
            <FaEye size={14} />
          </button>
        </div>
        ...
        {plan.type !== 'one-time' && plan.type !== 'emergency' && priceBreakdown && (
          <div className="mobsubplan-plan-delivery-info">
            ...
            {priceBreakdown.hasInitialDelivery && (
              <div className="mobsubplan-initial-note">Includes 1 initial delivery on day 1</div>
            )}
          </div>
        )}

        <div className="mobsubplan-plan-option-section">
          <label className="mobsubplan-option-label"><FaGasPump size={14} /> Cylinder Size</label>
          {(plan.type === 'custom' || plan.type === 'one-time' || plan.type === 'emergency') ? (
            <select className="mobsubplan-option-select" value={currentSize}
              onChange={(e) => setSelectedSize(prev => ({ ...prev, [plan._id]: e.target.value }))}>
              {sizeOptions.map(size => <option key={size} value={size}>{size}</option>)}
            </select>
          ) : (
            <div className="mobsubplan-option-info"><span>{plan.baseSize}</span></div>
          )}
        </div>

        {(plan.type === 'custom' || plan.type === 'preset') && (
          <div className="mobsubplan-plan-option-section">
            <label className="mobsubplan-option-label"><FaCalendar size={14} /> Delivery Frequency</label>
            <select className="mobsubplan-option-select" value={currentFrequency}
              onChange={(e) => setSelectedFrequency(prev => ({ ...prev, [plan._id]: e.target.value }))}>
        ...
        {plan.type !== 'one-time' && plan.subscriptionPeriod?.length > 0 && (
          <div className="mobsubplan-plan-option-section">
            <label className="mobsubplan-option-label"><FaCalendar size={14} /> Subscription Period</label>
            <select className="mobsubplan-option-select" value={currentPeriod}
              onChange={(e) => setSelectedPeriod(prev => ({ ...prev, [plan._id]: parseInt(e.target.value) }))}>
              {plan.subscriptionPeriod?.map(period => <option key={period} value={period}>{period} month{period > 1 ? 's' : ''}</option>)}
            </select>
          </div>
        )}

        <button className="mobsubplan-select-plan-btn" style={{ backgroundColor: getPlanColor(plan.type) }}
          onClick={() => handlePlanSelection(plan)} disabled={isProcessing}>
          {isProcessing ? 'Processing...' : 
           plan.type === 'one-time' ? 'Buy Now' :
           plan.type === 'emergency' ? 'Request Delivery' : 'Subscribe Now'}
          <FaArrowRight className="mobsubplan-btn-icon" />
        </button>
      </div>
    );
        ...
        <div className="mobsubplan-modal-content">
          <div className="mobsubplan-modal-header">
            <div className="mobsubplan-modal-title-section">
              ...
              <div>
                <h2>{planDetailsModal.name}</h2>
                <span className="mobsubplan-modal-plan-type" style={{ backgroundColor: getPlanColor(planDetailsModal.type) }}>
                  {planDetailsModal.type === 'one-time' ? 'One-Time Purchase' : 
                   planDetailsModal.type === 'emergency' ? 'Emergency Delivery' : 
                   planDetailsModal.type === 'custom' ? 'Custom Plan' : 'Subscription Plan'}
                </span>
            ...
            <button className="mobsubplan-close-modal" onClick={() => setPlanDetailsModal(null)}><FaTimes /></button>
          </div>
          <div className="mobsubplan-modal-body">
            <div className="mobsubplan-modal-section">
              <h3>Plan Description</h3>
            ...
            {planDetailsModal.features?.length > 0 && (
              <div className="mobsubplan-modal-section">
                ...
                <div className="mobsubplan-features-list">
                  {planDetailsModal.features.map((feature, index) => (
                    <div key={index} className={`mobsubplan-feature-item ${feature.included ? 'mobsubplan-included' : 'mobsubplan-excluded'}`}>
                      {feature.included ? <FaCheck className="mobsubplan-feature-icon" /> : <FaTimes className="mobsubplan-feature-icon" />}
                      <div className="mobsubplan-feature-content"><strong>{feature.title}:</strong> {feature.description}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mobsubplan-modal-section">
              <h3>Specifications</h3>
              <div className="mobsubplan-specifications-grid">
                <div className="mobsubplan-spec-item"><span className="mobsubplan-spec-label">Base Size:</span><span className="mobsubplan-spec-value">{planDetailsModal.baseSize}</span></div>
                <div className="mobsubplan-spec-item"><span className="mobsubplan-spec-label">Price per Kg:</span><span className="mobsubplan-spec-value">{formatCurrency(planDetailsModal.pricePerKg)}</span></div>
                {planDetailsModal.deliveryFrequency && (<div className="mobsubplan-spec-item"><span className="mobsubplan-spec-label">Delivery Frequency:</span><span className="mobsubplan-spec-value">{planDetailsModal.deliveryFrequency.join(', ')}</span></div>)}
                {planDetailsModal.subscriptionPeriod && (<div className="mobsubplan-spec-item"><span className="mobsubplan-spec-label">Subscription Period:</span><span className="mobsubplan-spec-value">{planDetailsModal.subscriptionPeriod.map(p => `${p} month${p > 1 ? 's' : ''}`).join(', ')}</span></div>)}
                {planDetailsModal.cylinderSizes && (<div className="mobsubplan-spec-item"><span className="mobsubplan-spec-label">Available Sizes:</span><span className="mobsubplan-spec-value">{planDetailsModal.cylinderSizes.join(', ')}</span></div>)}
                {planDetailsModal.deliveryInfo && (<>
                  <div className="mobsubplan-spec-item"><span className="mobsubplan-spec-label">Delivery Time:</span><span className="mobsubplan-spec-value">{planDetailsModal.deliveryInfo.deliveryTime}</span></div>
                  <div className="mobsubplan-spec-item"><span className="mobsubplan-spec-label">Free Delivery:</span><span className="mobsubplan-spec-value">{planDetailsModal.deliveryInfo.freeDelivery ? 'Yes' : 'No'}</span></div>
                </>)}
              </div>
            </div>
          </div>
          <div className="mobsubplan-modal-footer">
            <button className="mobsubplan-btn-secondary" onClick={() => setPlanDetailsModal(null)}>Close</button>
            <button className="mobsubplan-btn-primary" style={{ backgroundColor: getPlanColor(planDetailsModal.type) }}
              onClick={() => { setPlanDetailsModal(null); handlePlanSelection(planDetailsModal); }}>
              Select This Plan
            </button>
          </div>
        </div>
  ...
  return (
    <div className="mobsubplan-mobile-subscription-plans">
      ...
      <div className="mobsubplan-plan-category-section">
        <div className="mobsubplan-category-header"><FaTag className="mobsubplan-category-icon" /><div><h2>One-Time Purchase</h2><p className="mobsubplan-category-description">Perfect for occasional users. Purchase LPG gas as needed without any subscription.</p></div></div>
        <div className="mobsubplan-plans-list">{plans.filter(p => p.type === 'one-time').map(plan => renderPlanCard(plan))}</div>
      </div>

      <div className="mobsubplan-plan-category-section">
        <div className="mobsubplan-category-header"><FaBolt className="mobsubplan-category-icon" /><div><h2>Emergency Delivery</h2><p className="mobsubplan-category-description">Need gas urgently? Get immediate refill delivery within hours. Perfect for unexpected situations when you run out of gas.</p></div></div>
        <div className="mobsubplan-plans-list">{plans.filter(p => p.type === 'emergency').map(plan => renderPlanCard(plan))}</div>
      </div>

      <div className="mobsubplan-plan-category-section">
        <div className="mobsubplan-category-header"><FaFire className="mobsubplan-category-icon" /><div><h2>Regular Subscription Plans</h2><p className="mobsubplan-category-description">Choose from our flexible subscription plans for regular gas refill delivery. Save money and time with automatic deliveries based on your usage pattern.</p></div></div>
        <div className="mobsubplan-plans-list">{plans.filter(p => p.type === 'preset').map(plan => renderPlanCard(plan))}</div>
      </div>

      <div className="mobsubplan-plan-category-section">
        <div className="mobsubplan-category-header"><FaGasPump className="mobsubplan-category-icon" /><div><h2>Build Your Own Plan</h2><p className="mobsubplan-category-description">Customize every aspect of your gas delivery. Choose your cylinder size, delivery frequency, and subscription period.</p></div></div>
        <div className="mobsubplan-plans-list">{plans.filter(p => p.type === 'custom').map(plan => renderPlanCard(plan))}</div>
      </div>
      ...
      {showSummary && selectedPlan && (
        <div className="mobsubplan-plan-summary-modal">
          <div className="mobsubplan-summary-content">
            <div className="mobsubplan-summary-header">
              <h2>Confirm Your Plan</h2>
              <button className="mobsubplan-close-summary" onClick={() => setShowSummary(false)} disabled={isProcessing}><FaTimes /></button>
            </div>
            <div className="mobsubplan-summary-body">
              <div className="mobsubplan-plan-summary">
                <div className="mobsubplan-plan-summary-header">
                  {getPlanIcon(selectedPlan.type)}
                  <div><h3>{selectedPlan.name}</h3><span className="mobsubplan-plan-type">{selectedPlan.type}</span></div>
                </div>
                <div className="mobsubplan-summary-details">
                  <div className="mobsubplan-detail-item"><span className="mobsubplan-detail-label">Cylinder Size:</span><span className="mobsubplan-detail-value">{selectedPlan.selectedSize}</span></div>
                  <div className="mobsubplan-detail-item"><span className="mobsubplan-detail-label">Delivery Frequency:</span><span className="mobsubplan-detail-value">{selectedPlan.selectedFrequency}</span></div>
                  {selectedPlan.selectedPeriod > 1 && (<div className="mobsubplan-detail-item"><span className="mobsubplan-detail-label">Subscription Period:</span><span className="mobsubplan-detail-value">{selectedPlan.selectedPeriod} month{selectedPlan.selectedPeriod > 1 ? 's' : ''}</span></div>)}
                  {selectedPlan.priceBreakdown && (<div className="mobsubplan-detail-item mobsubplan-price-breakdown"><span className="mobsubplan-detail-label">Calculation:</span><div className="mobsubplan-breakdown-details"><div style={{ fontSize: '0.85rem', color: '#7f8c8d' }}>{selectedPlan.selectedSize} × ₦{selectedPlan.priceBreakdown.pricePerKg?.toLocaleString()}/kg × {selectedPlan.priceBreakdown.totalDeliveries} deliveries</div>{selectedPlan.priceBreakdown.breakdown.map((item, index) => (<div key={index} style={{ fontSize: '0.85rem', color: '#7f8c8d', marginTop: '2px' }}>{item}</div>))}</div></div>)}
                  <div className="mobsubplan-detail-item mobsubplan-total"><span className="mobsubplan-detail-label">Total Price:</span><span className="mobsubplan-detail-value mobsubplan-price">{formatCurrency(selectedPlan.price)}</span></div>
                </div>
              </div>
              <div className="mobsubplan-payment-method">
                <h4>Payment Method</h4>
                <div className="mobsubplan-payment-options">
                  <div className={`mobsubplan-payment-option ${paymentMethod === 'paystack' ? 'mobsubplan-selected' : ''}`} onClick={() => setPaymentMethod('paystack')}>
                    <div className="mobsubplan-option-content"><FaCreditCard className="mobsubplan-option-icon mobsubplan-card" /><div className="mobsubplan-option-text"><span className="mobsubplan-option-title">Pay with Card</span><span className="mobsubplan-option-description">Secure payment via Paystack</span></div></div>
                    <div className="mobsubplan-option-check">{paymentMethod === 'paystack' && <FaCheck />}</div>
                  </div>
                  <div className={`mobsubplan-payment-option ${paymentMethod === 'wallet' ? 'mobsubplan-selected' : ''}`} onClick={() => setPaymentMethod('wallet')}>
                    <div className="mobsubplan-option-content"><FaWallet className="mobsubplan-option-icon mobsubplan-wallet" /><div className="mobsubplan-option-text"><span className="mobsubplan-option-title">Pay with Wallet</span><span className="mobsubplan-option-description">Balance: {formatCurrency(walletBalance)}</span></div></div>
                    <div className="mobsubplan-option-check">{paymentMethod === 'wallet' && <FaCheck />}</div>
                  </div>
                </div>
                {paymentMethod === 'wallet' && (
                  <div className={`mobsubplan-wallet-notice ${walletBalance < selectedPlan.price ? 'mobsubplan-wallet-warning' : 'mobsubplan-wallet-info'}`}>
                    {walletBalance < selectedPlan.price ? (<><FaExclamationTriangle /><span>Insufficient wallet balance. Top up or select card payment.</span></>) : (<><FaCheck /><span>Wallet payment will be validated securely by our system</span></>)}
                  </div>
                )}
              </div>
            </div>
            <div className="mobsubplan-summary-footer">
              <button className="mobsubplan-summary-btn mobsubplan-cancel" onClick={() => setShowSummary(false)} disabled={isProcessing}>Cancel</button>
              <button className="mobsubplan-summary-btn mobsubplan-confirm" onClick={() => initializePayment(selectedPlan)} disabled={isProcessing || (paymentMethod === 'wallet' && walletBalance < selectedPlan.price)} style={{ backgroundColor: getPlanColor(selectedPlan.type) }}>
                {isProcessing ? (<><div className="mobsubplan-processing-spinner"></div>Processing...</>) : paymentMethod === 'wallet' ? `Pay ${formatCurrency(selectedPlan.price)}` : 'Proceed to Payment'}
              </button>
            </div>
          </div>
        </div>
      ...
      {isProcessing && (<div className="mobsubplan-processing-overlay"><div className="mobsubplan-processing-spinner-large"></div><p>Processing your request...</p></div>)}
    </div>
  );
};
