...
const ForgotPassword = () => {
  ...
  const handleRequestReset = async (e) => {
    ...
    infoToast('Sending reset instructions...');
    ...
    try {
      ...
      successToast('Reset email sent successfully!');
      setStep(2);
      setCountdown(60); // 60 seconds cooldown
    } catch (error) {
      const errorMsg = error.message || 'Failed to send reset instructions';
  ...
  const handleResetPassword = async (e) => {
    ...
    setLoading(true);
    setMessage({ type: '', text: '' });
    infoToast('Resetting your password...');
  ...
  const handleResendToken = async () => {
    ...
    setLoading(true);
    infoToast('Resending reset token...');
    ...
    try {
      ...
      successToast('New token sent!');
      setCountdown(60);
    } catch (error) {
      const errorMsg = error.message || 'Failed to resend token';
      setMessage({ type: 'error', text: errorMsg });
      errorToast(errorMsg);
    } finally {
      setLoading(false);
